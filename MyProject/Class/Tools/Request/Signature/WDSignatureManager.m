...
#define APP_SECRET      @"515e5a339983463b9a727b6d9f20d207"         //APPsecret配置
...
#define API_VERSION     @"4.1.0"       //API版本配置
#define APP_NAME        @"com.wondersgroup.healthcloud.3101://user" //APP_Name配置
...
+ (void)addHeader:(AFHTTPRequestSerializer <AFURLRequestSerialization> *)requestSerializer
  ...
  isURLHaveAPITag:(BOOL)isURLHaveAPITag {
    ...
    NSDictionary *headerDict = @{
                                 ...
                                 @"main-area"     : MAIN_AREA,
                                 @"model"         : DEVICE_MODEL,
                                 @"os-version"    : DEVICE_VERSION,
                                 @"platform"      : PLATFORM,
                                 @"screen-height" : SCREEN_HEIGHT_STR,
                                 @"screen-width"  : SCREEN_WIDTH_STR,
                                 @"spec-area"     : @"",
                                 @"version"       : API_VERSION,
                                 };
...
+ (NSString *)signatureStringWithHeaderDict:(NSDictionary *)headerDict
    ...
    if ([WDRequestManager manager].time_diff) {
        requestTime = [NSString stringWithFormat: @"%@+time-diff=%@", requestTime, [WDRequestManager manager].time_diff];
    }
